...
const getHeader = ({ ext = [], int = [] }) => {
  ...
  const getReactImports = extOptions => {
    const reqList = [];
    extOptions.forEach(
      selected =>
        options.ext.effects[selected] &&
        reqList.push(options.ext.effects[selected])
    );
    return reqList.length
      ? `import React, { ${reqList.join(', ')} } from 'react';`
      : '';
  };
 ...
 * @param {Array} [params.ext] external imports
 */
const getComponent = ({ name, ext = [] }) => {
  const options = {
    useRef: `const ${name}Ref = useRef();`,
    useState: `const [value, updateValue] = useState();`,
    useEffect: `useEffect(() => () => {}, []);`,
  };
